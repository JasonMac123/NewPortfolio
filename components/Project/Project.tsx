...
const Project = ({ title, description, tags, image, link }: ProjectProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="pt-4 pb-8 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:w-1/2 flex flex-col h-full group-even:ml-[19rem]">
          ...
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {tags.map((tag, i) => {
              return (
                <li
                  className="bg-[#0077b6]/[0.7] px-3 py-2 text-[0.7rem] uppercase tracking-wider text-black rounded-3xl"
                  key={i}
                >
                  {tag}
                </li>
